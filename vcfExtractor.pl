...
#   03/01/2014 (v3.4.0)  - Was using less than ideal algorithm to pull our VCF metrics for VAF
#                          and Coverage stats.  Use Jingwei's suggestions to parse these out in
#                          a better way.
#
# D Sims 2/21/2013
#################################################################################################
        ...
        if ( $fro eq '.' ) {
            ...
            for my $alt_var ( @alt_array ) {
                ...
                for my $index ( @array_pos ) {
                    ...
                    # XXX Sometimes there is only 1 FAO field even though there should be more.  Set $fao_array[$index] to 0
                    #if ( ! defined $fao_array[$index] ) { print "$var_id\n"; next; }
                    $fao_array[$index] = 0 if ( ! defined $fao_array[$index] );

                    my $vaf = vaf_calc( \$reason, \$tot_coverage, \$fro, \$fao_array[$index] );
                    next if ( $noref && $vaf == 0 );
                    my $cosid = $oid_array[$index];
                    push( @{$parsed_data{$var_id}}, $parsed_pos, $oref_array[$index], $oalt_array[$index], $filter, $reason, $gtr, $vaf, $tot_coverage, $fro, $fao_array[$index], $oid_array[$index] );
                }
            }
        }
    }
...
sub format_output {
    ...
    for my $variant ( sort { versioncmp( $a, $b ) }  keys %$data ) {
        printf $out_fh $format, @{$$data{$variant}}[0,1,2,3,4,6,7,8,9,10];
    }
}
